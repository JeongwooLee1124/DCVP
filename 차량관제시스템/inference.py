...
def infer():
    ...
    sr_model = load_pretrained_state_dict(sr_model, False, "./results/pretrained_models/SRGAN_x4-ImageNet.pth.tar")
    print("Load model weights successfully.")
    
    sr_model.eval()
    sr_model.half()
    for i, path in enumerate(lr_path):
        input_tensor = preprocess_one_image(path, False, True, device)
        ...
        cv2.imwrite(f'./sr/{i}.png', cr_image)

        print(f"SR image save to `./sr/{i}.png`")
        
        files = [
          ('file', open(f'./sr/{i}.png','rb'))
        ]
        headers = {
          'X-OCR-SECRET': secret_key
        }
        
        print("Load SR image to OCR")
        
        response = requests.request("POST", api_url, headers=headers, data = payload, files = files)
        res = json.loads(response.text.encode('utf8'))
        txt = res['images'][0]['fields'][0]['inferText'] + res['images'][0]['fields'][1]['inferText']
        print()
        print(txt)
        print()
        
        with open(f'./ocr/{i}.json', 'w', encoding='utf-8') as outfile:
            json.dump(res, outfile, indent=4, ensure_ascii=False)
            
        print(f"OCR results save to `./ocr/{i}.json`")
# def main(args):
